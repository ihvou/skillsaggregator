...
- [x] 14. **Build 16** — built from `8af73a9`, uploaded and processed (`VALID`) on 2026-09-18; export compliance answered by `ITSAppUsesNonExemptEncryption=false`. (API) **iPhone-only** (`supportsTablet: false`). With iPad on, App Review demands iPad screenshots and tests on an iPad, and iPad support can never be removed from later versions. Build and upload per *Runbook A2*; wait for `processingState=VALID`.
...
- [x] 16. **Attach build 16** — done through the API on 2026-09-18 (it replaced build 15). By hand: **Distribution** tab → iOS App → **1.0 Prepare for Submission** → *Build* section → remove the old build, add the new one.
...
- [x] 18. **Submitted for review on 2026-09-19** — version 1.0, build 16, manual release; state `WAITING_FOR_REVIEW` (API). **Add for Review** → **Submit for Review**, top right of the same page. **Not** the TestFlight tab: its "Submit for Review" only sends a build to *Beta* App Review.
